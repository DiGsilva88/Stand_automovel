@extends('layouts.app')
@section('title', 'Editar Venda #' . str_pad($venda->id, 4, '0', STR_PAD_LEFT) . ' — SS Motors')
...
<main class="px-6 md:px-20 pt-28 md:pt-36 pb-24 max-w-[1440px] mx-auto bg-[#131313]">
    ...
    <header class="mb-12 flex justify-between items-end flex-wrap gap-4 pb-6 border-b border-white/5">
        <div>
            ...
            <h1 class="text-4xl md:text-5xl font-bold text-white uppercase tracking-tighter" style="font-family: 'Sora', sans-serif;">Editar Transação</h1>
            <p class="text-xs font-mono text-[#8e90a2] uppercase tracking-wider mt-1">Atualize as informações contratuais do registo #{{ str_pad($venda->id, 4, '0', STR_PAD_LEFT) }}.</p>
        </div>
        ...
        <a href="{{ route('vendas.index') }}"
           class="font-mono text-xs text-[#8e90a2] hover:text-white border border-white/10 px-5 py-3 flex items-center gap-2 uppercase tracking-widest transition-all rounded-sm">
            <span class="material-symbols-outlined text-sm">arrow_back</span> Cancelar
        </a>
    </header>
    ...
    <div class="max-w-3xl bg-[#141313] border border-white/5 rounded-sm p-6 md:p-10 shadow-2xl">
        ...
        <form action="{{ route('vendas.update', $venda->id) }}" method="POST" class="space-y-6">
            ...
            <div class="flex flex-col space-y-2">
                <label for="viatura_id" class="font-mono text-[10px] text-[#8e90a2] uppercase tracking-widest font-medium">Viatura Comercializada</label>
                <select name="viatura_id" id="viatura_id" required
                    ...
                    @foreach($viaturas as $viatura)
                        <option value="{{ $viatura->id }}" class="bg-[#141313]" {{ $venda->viatura_id == $viatura->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <!-- Seleção de Cliente -->
                <div class="flex flex-col space-y-2">
                    <label for="cliente_id" class="font-mono text-[10px] text-[#8e90a2] uppercase tracking-widest font-medium">Cliente Adquirente</label>
                    <select name="cliente_id" id="cliente_id" required
                            class="w-full bg-[#1a1a1a] border border-white/10 rounded-sm px-4 py-3 text-sm text-white focus:outline-none focus:border-[#b8c3ff]/50 font-sora transition-colors">
                        @foreach($clientes as $cliente)
                            <option value="{{ $cliente->id }}" class="bg-[#141313]" {{ $venda->cliente_id == $cliente->id ? 'selected' : '' }}>
                                {{ $cliente->nome }}
                            </option>
                        @endforeach
                    </select>
